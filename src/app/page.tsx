  ...
  return (
    <div className="landing min-h-dvh">
      ...
      <header className="site-nav glass">
        <div className="landing-inner flex items-center gap-3 py-4">
          ...
          <nav className="ml-auto flex min-w-0 flex-wrap items-center justify-end gap-1" aria-label="Primary">
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <section className="landing-inner landing-hero">
          <div className="min-w-0">
            ...
            <ul className="reveal reveal-d4 mt-8 flex flex-wrap items-center gap-x-5 gap-y-2 text-small text-muted">
          ...
          <MotionGate className="min-w-0 reveal reveal-quick reveal-d5 lg:justify-self-end lg:w-full">
            <div className="card-elevated lift lift-strong showcase" aria-hidden="true">
              <div className="showcase-head">
                ...
                <span className="chip ml-auto font-mono tabular-nums">03 / 06</span>
              </div>
              <h2 className="panel-title mt-4">Questioning — retrieval check</h2>
              <div className="showcase-body">
                <div className="rail">
                  {arc.map((node, index) => (
                    <div className="rail-node" data-state={node.state} key={node.label}>
                      <span className="rail-dot">
                        {node.state === "done" ? <Icon name="check" /> : index + 1}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mock-bubble mock-bubble-ai">
                  <div className="mock-line skeleton w-11/12" />
                  <div className="mock-line skeleton mt-2 w-4/5" />
                  <div className="mock-line skeleton mt-2 w-3/5" />
                </div>
              </div>
            </div>
          </MotionGate>
        ...
        {/* Pillars ------------------------------------------------------- */}
        <section className="landing-inner py-10">
          ...
          <div className="reveal-stagger mt-8 grid gap-4 sm:grid-cols-3">
            {pillars.map((pillar, index) => (
              <article key={pillar.title} className="card lift panel min-w-0" data-reveal>
        ...
        <section className="landing-inner py-10">
          ...
          <dl className="reveal-stagger mt-8 grid gap-4 sm:grid-cols-2">
            {facts.map(([term, description], index) => (
              <div key={term} className="card lift panel min-w-0" data-reveal>
                <dt className="flex items-baseline gap-2">
                  <span className="index-mark">0{index + 1}</span>
                  <span className="panel-title">{term}</span>
                </dt>
                <dd className="panel-body mt-2">{description}</dd>
              </div>
            ))}
          </dl>
        </section>
        ...
        <section className="landing-inner pb-10 pt-16">
